...
const actions: ActionTree<any, any> = {
  async setUser({ state, commit }, user: any) {
    ...
    if (user) {
      ...
      // const { key } = decoded;
      // 返回当前用户拥有权限的路由
      let accessedRouters = filterAsyncRouter(asyncRouterMap, u.userName);
      console.log('accessedRouter-----s', accessedRouters);
  ...
  // {key:对象标识,data:对象值}
  async setData({ state, commit }, data: any) {
    if (data.data) {
      commit('SET_DATA', data);
    }
  },
  async setClientHeight({ state, commit }, data: any) {
    commit('setClientHeight', data);
  }
};
